...
const Goals = () => {
  ...
  const loadGoals = async () => {
    try {
      ...
      const goalsData = normalizeGoals(response);
      setGoals(goalsData);
      setError(null);
    } catch (err) {
  ...
  const updateStats = useCallback(() => {
    ...
    const completed = validGoals.filter(goal => goal.status === 'completed' || goal.status === 'COMPLETED').length;
    const inProgress = validGoals.filter(goal => goal.status === 'in-progress' || goal.status === 'IN_PROGRESS').length;
    const notStarted = validGoals.filter(goal => goal.status === 'not-started' || goal.status === 'NOT_STARTED').length;
    const totalProgressUnits = validGoals.reduce((sum, goal) => sum + (goal.progress || 0), 0);
    const maxPossibleProgress = total * 100;
    const overallProgressPercentage = maxPossibleProgress > 0 
      ? Math.round((totalProgressUnits / maxPossibleProgress) * 100) 
      : 0;
    setStats({
      total,
      completed,
  ...
  // Robust! Never calls .trim() on undefined
  const isFormValid = () => {
    return (
      ((newGoal.title || '').trim()) &&
  ...
  const handleUpdateGoal = async (updatedGoal) => {
    try {
      ...
      const updated = await goalsAPI.update(updatedGoal.id, goalData);
  ...
  const handleStatusChange = (goalId, newStatus) => {
    setGoals(goals.map(goal => {
      if (goal.id === goalId) {
        ...
        else if (newStatus === 'in-progress' && newProgress === 0) newProgress = 25;
        else if (newStatus === 'increment') newProgress = Math.min(newProgress + 25, 100);
  ...
  const incrementProgress = async (goalId) => {
    try {
      ...
      if (goal) {
        ...
        const updatedGoal = goalsAPI.updateProgress
          ? await goalsAPI.updateProgress(goalId, newProgress)
          : await goalsAPI.update(goalId, { ...goal, progress: newProgress });
  ...
  const getFilteredGoals = () => {
    const validGoals = Array.isArray(goals) ? goals : [];
    if (filter === 'all') return validGoals;
    return validGoals.filter(goal => {
  ...
  const filteredGoals = Array.isArray(getFilteredGoals()) ? getFilteredGoals() : [];
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                          <Box sx={{ flex: 1 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                              <Chip
                                label={goal.status ? String(goal.status).replace(/[-_]/g, " ") : ""}
